...
class CommonExpensePeriodPerformanceTest extends TestCase
{
    ...
    public function generate_mass_billing_no_n_plus_one_queries()
    {
        ...
        // 5 base + 1 por propiedad (updateOrCreate) + pequeño margen por whereHas/grupos
        $expectedMaxQueries = 5 + $propertyCount + 6;
        $this->assertLessThanOrEqual($expectedMaxQueries, $queryCount,
            "Se ejecutaron {$queryCount} queries SELECT. Esperado ≤ {$expectedMaxQueries} (5 base + {$propertyCount} updateOrCreate + margen). El bucle foreach NO debería hacer query extra por propiedad para previous_balance.");
    }
