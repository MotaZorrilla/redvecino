...
import { LayoutDashboard, Building2, Users, Wrench, CircleDollarSign, Scale, Settings, PanelLeftClose, PanelLeft } from 'lucide-react';
...
export default function AdminLayout({
    ...
    const tabLabels = {
        dashboard: <><LayoutDashboard className="w-5 h-5 text-indigo-500" /> Panel de Resumen</>,
        properties: <><Building2 className="w-5 h-5 text-indigo-500" /> Propiedades de la Comunidad</>,
        users: <><Users className="w-5 h-5 text-indigo-500" /> Usuarios y Copropietarios</>,
        tickets: <><Wrench className="w-5 h-5 text-indigo-500" /> Gestión de Tickets de Soporte</>,
        payments: <><CircleDollarSign className="w-5 h-5 text-indigo-500" /> Registro de Pagos e Ingresos</>,
        fines: <><Scale className="w-5 h-5 text-indigo-500" /> Control de Multas y Sanciones</>,
        settings: <><Settings className="w-5 h-5 text-indigo-500" /> Configuración y Ajustes</>,
    };
            ...
            <aside aria-label="Navegación principal" className={`${SIDEBAR_W} bg-slate-950 text-white flex flex-col justify-between shrink-0 font-sans transition-all duration-300 fixed inset-y-0 left-0 z-30 overflow-hidden ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                    ...
                    <nav aria-label="Menú principal" className="space-y-1">
                        {tabs.map(tab => (
                            <button
                                ...
                                aria-current={adminActiveTab === tab.id ? 'page' : undefined}
                ...
                <div className="p-4">
                    <button
                        ...
                        title={sidebarCollapsed ? `${adminSettingsForm.name} — Configurar` : undefined}
                        className={`w-full transition-all duration-200 p-3 rounded-2xl space-y-2 text-left relative group ${
                            sidebarCollapsed ? 'flex justify-center' : ''
                        } ${
                            adminActiveTab === 'settings'
                                ? 'bg-slate-900 border border-indigo-500 shadow-md ring-1 ring-indigo-500/25'
                                : 'bg-slate-900/60 border border-slate-800/80 hover:bg-slate-900/90 hover:border-slate-700'
                        }`}
                    >
                        <div className={`flex items-center gap-2.5 ${sidebarCollapsed ? 'justify-center' : ''}`}>
                            ...
                            {!sidebarCollapsed && (
                                <div className="min-w-0 flex-1">
                                    ...
                                    <span className="text-[9px] text-slate-500 block truncate font-medium">Administrador (Configurar)</span>
                                </div>
                            )}
                        </div>
                        {!sidebarCollapsed && (
                            <div className="pt-1.5 border-t border-slate-800/60 flex justify-between items-center text-[9px] text-slate-400 group-hover:text-slate-300 transition-colors">
                                ...
                                <span aria-live="polite" className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                            </div>
                        )}
                    </button>
                </div>
                ...
                <header style={{ paddingTop: 'env(safe-area-inset-top)' }} className={`h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-150 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 lg:px-8 fixed top-0 right-0 left-0 ${CONTENT_PL} z-20 transition-all duration-300`}>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        <div className="flex items-center gap-2 bg-gray-100 dark:bg-slate-950/40 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-slate-800">
                            ...
                            <span className="text-[8px] bg-indigo-500/10 border border-indigo-500/30 text-indigo-500 font-extrabold uppercase px-1.5 py-0.5 rounded hidden sm:inline">ADMIN</span>
                        </div>
