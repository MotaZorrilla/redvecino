...
import { Clock, Package, UserCheck, FileText, ShoppingCart, Wrench, CalendarClock, PanelLeftClose, PanelLeft } from 'lucide-react';
...
export default function ColaboradorLayout({
    ...
    adminCondoId,
    setAdminCondoId,
    colaboradorActiveTab,
    setColaboradorActiveTab,
    isMobileSidebarOpen,
    ...
    const tabs = [
        { id: 'attendance', icon: Clock, label: 'Control Asistencia', desc: 'Registro entrada y salida' },
        { id: 'shifts', icon: CalendarClock, label: 'Turnos y Horarios', desc: 'Bitácora y novedades' },
        { id: 'packages', icon: Package, label: 'Encomiendas OCR', desc: 'Custodia e inventario' },
        { id: 'visitors', icon: UserCheck, label: 'Registro de Visitas', desc: 'Accesos peatonales/portón' },
        { id: 'contracts', icon: FileText, label: 'Contratos y Liquidaciones', desc: 'Contrato laboral y sueldo' },
        { id: 'shopping', icon: ShoppingCart, label: 'Lista de Compras', desc: 'Insumos y suministros' },
        { id: 'tickets', icon: Wrench, label: 'Incidencias Asignadas', desc: 'Mantenimiento del condominio' },
    ];

    const tabLabels = {
        attendance: <><Clock className="w-5 h-5 text-indigo-500" /> Control de Asistencia</>,
        shifts: <><CalendarClock className="w-5 h-5 text-indigo-500" /> Turnos y Horarios</>,
        packages: <><Package className="w-5 h-5 text-indigo-500" /> Custodia de Encomiendas</>,
        visitors: <><UserCheck className="w-5 h-5 text-indigo-500" /> Control de Accesos</>,
        contracts: <><FileText className="w-5 h-5 text-indigo-500" /> Contratos y Liquidaciones</>,
        shopping: <><ShoppingCart className="w-5 h-5 text-indigo-500" /> Lista de Compras</>,
        tickets: <><Wrench className="w-5 h-5 text-indigo-500" /> Tareas de Mantenimiento</>,
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex flex-col md:flex-row relative w-full font-sans text-gray-700 dark:text-slate-200">
            <Head>
                <title>Portal Colaborador - Estación de Conserjería</title>
                <meta name="description" content="Portal del Colaborador y Conserjería de RedVecino." />
            </Head>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {!sidebarCollapsed && (
                            <div className="text-left min-w-0">
                                ...
                                <p className="text-[9px] text-slate-400 uppercase tracking-widest font-mono mt-1">Conserjería</p>
                            </div>
                        )}
                        <button
                            type="button"
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setColaboradorActiveTab('packages');
                        ...
                        {tabs.map(tab => (
                            ...
                            >
                                <div className={`flex items-center gap-2 ${sidebarCollapsed ? 'justify-center' : ''}`}>
                                    <tab.icon className={`w-4 h-4 ${colaboradorActiveTab === tab.id ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-300'}`} />
                                    {!sidebarCollapsed && (
                                        <span className={`text-xs font-bold ${colaboradorActiveTab === tab.id ? 'text-indigo-400' : 'text-slate-300 group-hover:text-slate-200'}`}>
                                            {tab.label}
                                        </span>
                                    )}
                                </div>
                        ...
                        <div className={`flex items-center gap-2.5 ${sidebarCollapsed ? 'justify-center' : ''}`}>
                            ...
                            {!sidebarCollapsed && (
                                <div className="min-w-0 flex-1">
                                    ...
                                    <span className="text-[9px] text-slate-500 block truncate font-medium">Conserje / Colaborador</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <header style={{ paddingTop: 'env(safe-area-inset-top)' }} className={`h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-150 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 lg:px-8 fixed top-0 right-0 left-0 ${CONTENT_PL} z-20 transition-all duration-300`}>
                    <div className="flex items-center gap-3 text-left">
                        ...
                        <h2 className="text-xs sm:text-sm font-black text-gray-900 dark:text-slate-100 uppercase tracking-wider flex items-center gap-2">
                            {tabLabels[colaboradorActiveTab]}
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        <div className="flex items-center gap-2 bg-gray-100 dark:bg-slate-950/40 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-slate-800">
                            <div className="h-6 w-6 rounded bg-indigo-650 flex items-center justify-center font-bold text-white text-[10px]">
                                {user?.name?.charAt(0) || 'CO'}
                            </div>
                            <span className="text-[11px] font-bold text-gray-700 dark:text-slate-350 hidden sm:inline">{user?.name}</span>
                            <span className="text-[8px] bg-indigo-500/10 border border-indigo-500/30 text-indigo-500 font-extrabold uppercase px-1.5 py-0.5 rounded hidden sm:inline">STAFF</span>
                        </div>
