...
| HF-03 | `ReferenceError: Cannot access 'canViewFinances' before initialization` | Variable `const` en TDZ: declarada despuÃ©s del `useEffect` que la referenciaba en su dependency array | Reemplazada variable externa por chequeo inline dentro del callback del effect |

### 3.18 IntegraciÃ³n y AnÃ¡lisis Contable/Laboral zAux (Junio 2026)

Se realizÃ³ una revisiÃ³n minuciosa y especificaciÃ³n tÃ©cnica basada en los nuevos archivos financieros provistos en la carpeta [zAux](file:///C:/xampp/htdocs/redvecino/zAux):
*   **AnÃ¡lisis Contable de Gastos Comunes:** Se incorporÃ³ el algoritmo de base distribuible ($E_{total} - I_{total}$), la categorizaciÃ³n de movimientos, los 5 mÃ©todos de distribuciÃ³n y el prorrateo de Fondo de Reserva sobre el subtotal de la unidad habitacional.
*   **AnÃ¡lisis Laboral de Remuneraciones:** Se estructurÃ³ la jerarquÃ­a de haberes (imponibles y no imponibles), cotizaciones previsionales chilenas (salud 7%, pensiÃ³n dinÃ¡mica, seguro cesantÃ­a 0.6%) y otros descuentos del sueldo lÃ­quido.
*   **ActualizaciÃ³n de Documentos:** Se registraron todas las especificaciones y fÃ³rmulas matemÃ¡ticas en la secciÃ³n 15.13 de [SPEC.md](file:///C:/xampp/htdocs/redvecino/SPEC.md), y se definieron las tareas de desarrollo en la secciÃ³n 2.8 de este documento.

### 3.19 Hardening de QA y ExpansiÃ³n de Tests (SesiÃ³n 18/06/2026)

Se implementÃ³ el plan de robustecimiento y expansiÃ³n de pruebas automatizadas:
*   **Hardening de Pruebas Financieras:** Agregados casos de prueba para desbordamientos extremos (lÃ­mites a nivel de miles de millones), valores negativos y nulos en el cÃ¡lculo de gastos comunes, y validaciones de prorrateo por defecto (fallbacks A/B/C) en `AdvancedFinancesAndPayrollTest.php`.
*   **Hardening de Incidencias:** Enlazada la validaciÃ³n de evidencia fotogrÃ¡fica obligatoria antes de permitir que un colaborador marque un ticket como resuelto en `TicketController.php`. Actualizados los tests existentes y aÃ±adida la verificaciÃ³n de bloqueo en `TicketLifecycleTest.php` e `IncidenciasLifecycleTest.php`.
*   **Nuevas Funcionalidades y Tests:** DiseÃ±ada y ejecutada una migraciÃ³n de base de datos (`2026_06_18_000000_add_pin_and_bookings_to_database.php`) para aÃ±adir PIN de seguridad, reservas, invitaciones de visitas y control de correspondencia. Desarrollada la lÃ³gica del backend en `RoadmapFeaturesController.php` y enlazadas las APIs en `routes/api.php`.
*   **CertificaciÃ³n QA de la Suite:** Implementado el archivo de pruebas `RoadmapFeaturesTest.php` para cubrir PIN auth, alertas de morosidad ($\ge 3$ meses impagos bloquea reservas), pases de visitas QR de un solo uso, custodia de correspondencia con firma base64, quÃ³rum de asambleas ponderado por coeficientes y protecciÃ³n del fondo de reserva.
*   **Resultado Exitoso:** Todos los tests pasan correctamente. Total suite: **193 tests, 890 aserciones pasadas en 1:46s al 100% de Ã©xito.**

### 3.20 Hardening de Tests con Pest v3 y Coexistencia de Suites (Sesión 04/07/2026)

Se implementó una expansión masiva de la suite de pruebas utilizando Pest v3 como framework secundario, coexistiendo con PHPUnit existente:

*   **Instalación de Pest v3.8.6:** Se instaló `pestphp/pest` v3.8.6 y `pestphp/pest-plugin-laravel` v3.2.0 vía Composer, sin conflictos con PHPUnit existente.
*   **6 Nuevas Suites de Pruebas Agresivas en Sintaxis Pest:**
    *   `FinanzasConsistenciaPest.php` (15 tests) — Montos límite ($0, negativos, no numéricos), categorías contables inválidas, subcategorías fuera de jerarquía, consistencia CRUD completa.
    *   `QuorumExtremosPest.php` (11 tests) — Quórum exacto al 50%, 49.99%, 0 asistentes, duplicados, cross-condo, fallback de coeficiente.
    *   `RBACMatrizCompletaPest.php` (12 tests) — Matriz exhaustiva de 6 roles contra ~10 endpoints administrativos con verificación de aislamiento.
    *   `AislamientoMultiCondoPest.php` (8 tests) — Aislamiento multi-condominio para tickets, incomes, expenses, pagos, multas, anuncios y common_expenses.
    *   `CoeficienteFallbackPest.php` (7 tests) — Cadena completa de fallback de coeficiente: coefficient → ownership_percentage → area_sqm → default.
    *   `ConcurrenciaFinancieraPest.php` (8 tests con `->repeat(3)`/`->repeat(5)`) — Stress de CRUD financiero concurrente para detectar race conditions.
*   **Fix de 13+ Fallos en Nuevos Tests Pest:**
    *   **Descubrimiento de archivos:** phpunit.xml no tenía `<directory suffix="Pest.php">` — agregado para que PHPUnit descubra archivos `*Pest.php`.
    *   **`uses()` duplicado:** `uses(Tests\TestCase::class)` repetido en archivos individuales chocaba con la declaración global en `tests/Pest.php` — eliminado de todos los archivos Pest.
    *   **Float vs Int:** `->toBe()` (identidad estricta `===`) fallaba comparando `80000.0` con `80000` — reemplazado por `->toEqual()` (igualdad suelta `==`) para valores decimales.
    *   **Datasets con `app()`:** Closures de dataset que llamaban `app()` o servicios se evaluaban en carga de clase antes del bootstrap de Laravel — reemplazados por bucles inline dentro del test.
    *   **`payment_method` faltante:** `Payment::create()` requiere `payment_method` — agregado en todos los factories y tests relevantes.
    *   **`created_by` faltante:** Tickets y announcements requieren `created_by` — agregado en creaciones directas.
    *   **Seeded data contaminante:** Cálculos que dependían de datos puros fallaban por registros residuales del `DatabaseSeeder` — agregada limpieza explícita en `beforeEach`.
    *   **Named parameters en datasets:** Pest requiere que keys de arrays asociativos en datasets coincidan exactamente con nombres de parámetros de la función de test — corregidos.
    *   **Roles sin acceso al catálogo:** Roles no autorizados (Colaborador, Propietario, Residente) esperaban 403 pero obtenían 200 — ajustadas aserciones.
*   **phpunit.xml:** Agregado `<directory suffix="Pest.php">tests/Feature</directory>` para descubrimiento de Pest files.
*   **Suite de Pruebas Final:**
    *   Con `./vendor/bin/pest`: **337 tests, 0 failures** (incluye PHPUnit + Pest).
    *   Con `php artisan test`: **336 tests, 0 failures** (excluye Unit/ExampleTest).
*   **Build Vite:** `npx vite build` compila exitosamente con 2814 modules, 0 errores.

### 3.21 Feature Expansion: Facilities, HR Liquidaciones, PersonWizard & Security Hardening (Sesión 04/07/2026)

Se completaron 4 frentes de trabajo en paralelo: arch() tests, frontend testing, features roadmap y hardening de seguridad:

#### arch() Tests y Frontend Testing
*   **arch() tests estructurales:** Creados 9 tests en `tests/Arch/ArchPest.php` validando naming de controllers, herencia de modelos (Eloquent), Services final, prohibición de debug calls (`dd`/`dump`/`var_dump`), prohibición de DB facade directa en controllers y raw SQL queries.
*   **3 Services `final`:** `CommonExpenseCalculator`, `CondoFinanceService`, `PayrollCalculator` ahora son `final class`.
*   **Vitest + React Testing Library:** Instalado Vitest v4.1.9, @testing-library/react v16.3.2, jsdom v29.1.1. Configuración en `vitest.config.js` con alias `@` y setupFiles.
*   **29 tests frontend:** helpers.test.js (14), notify.test.js (5), constants.test.js (4), Toast.test.jsx (6) — todos pasando en ~1.5s.

#### Facility CRUD (Backend + Tests)
*   **FacilityController** con métodos store/show/update/destroy + validación de datos en español.
*   **FacilityFactory** con datos realistas (quincho, piscina, gimnasio, sala de eventos).
*   **12 tests Pest** en `FacilityCrudPest.php` — CRUD completo, permisos y validación.

#### HR Liquidaciones (Backend + Tests)
*   **HRController** extendido: showEmployee, updateEmployee, listLiquidations, showLiquidation, updateLiquidation, deleteLiquidation.
*   **LiquidationFactory** con todos los campos NOT NULL.
*   **10 tests Pest** en `HrCrudPest.php` — CRUD de empleados y liquidaciones.
*   Bug fix: `saveLiquidation` validaba solo 4 campos → agregados 8 campos adicionales requeridos.

#### PersonWizard Backend (API + Frontend)
